...
export async function waitCommand(options: WaitOptions): Promise<WaitResult> {
  ...
  const capturePane = options.capturePane ?? ((session: string, tail: number) => captureTmuxPane(session, tail, connection));
  const paneActivity = options.paneActivity ?? ((session: string) => probePaneActivity(session, { connection }));
